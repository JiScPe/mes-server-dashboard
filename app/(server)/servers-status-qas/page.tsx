...
import { groupServersByTypeQAS, SERVICE_TYPE_ORDER_QAS } from "@/lib/helpers/group-by-type-qas";
...
const ServerListPage = async ({ searchParams }: Props) => {
  ...
  const res = await fetch(
    `${API_URL}:${PORT}/api/status-all-qas?nginxserv=${queryParams}`,
    { cache: "no-store" },
  );

  const { servers }: iServersStatusResponse = await res.json();
  const groupedServers = groupServersByTypeQAS(servers);
    ...
    <ServerListProvider value={{ queryParams }}>
      <div className="min-h-screen">
        ...
        <main className="space-y-8 p-4 dark:bg-secondary">
          {SERVICE_TYPE_ORDER_QAS.map((type) => {
